...
layout: post
date: 2026-07-17
title: "微信（Weixin） | Hermes Agent"
source: "https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/weixin"
author:
published:
created: 2026-07-17
...
> 信息
> 
> 本适配器适用于 **个人微信账号** （微信）。如需对接企业微信，请参阅 [WeCom 适配器](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/wecom) 。

> [!-warning] -warning
> iLink bot 身份——普通微信群可能无法使用
> 
...
| `allow_from` | `[]` | 允许发送私信的用户 ID（当 dm\_policy=allowlist 时生效） |
| `group_allow_from` | `[]` | 允许的群组 ID（当 group\_policy=allowlist 时生效） |
...
| `WEIXIN_GROUP_ALLOWED_USERS` | — | *（空）* | 群组白名单的逗号分隔 **群聊 ID** （非成员用户 ID）。变量名为历史遗留，实际填写的是群 ID 而非用户 ID。 |
